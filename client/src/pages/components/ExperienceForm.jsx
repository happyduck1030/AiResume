...
const ExperienceForm = ({data,onChange}) => {
  ...
  const generateDescription = async (index) => {
    ...
    const prompt = `优化我的工作描述：${(experience.description)}，职位是 ${experience.position}，公司是 ${experience.company}`
    try{
    ...
    <div className='space-y-6'>
      <div>
        ...
        className='flex items-center gap-2 px-3 py-1 text-sm bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors '>
          <Plus className='size-4'></Plus>Add Experience
        </button>
      </div>
      {data.length===0?(
        ...
        <div className='space-y-4'>
          {data.map((experience,index)=>{
            ...
            <div className='grid md:grid-cols-2 gap-3'>
              ...
              <input value={experience.end_date || ''} disabled={experience.is_current} onChange={(e) => updateExperience(index, 'end_date', e.target.value)}
            ...
            <label>
              ...
              className='rounded bg-gray-300 text-blue-600 focus:ring-blue-500'
              />
            ...
            <div className='space-y-2'>
              <div className='flex items-center justify-between'>
                ...
                <button  disabled={generatingIndex===index || !experience.position||!experience.company} 
                onClick={()=>generateDescription(index)}
                 className='flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>
                 {generatingIndex===index?(<Loader2 className='w-3 h-3 animate-spin' />):(<Sparkles className='w-3 h-3 '/>)} Enhance with AI
                </button>
              </div>
              <textarea rows={4} value={experience.description||""}  onChange={(e)=>updateExperience(index,"description",e.target.value)}
              className='w-full text-sm px-3 py-2 rounded-lg resize-none' placeholder='Describe your key resposibilities and achievements...'></textarea>
            </div>
            </div>)
          })}
        </div>

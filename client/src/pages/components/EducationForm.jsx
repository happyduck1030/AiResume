      ...
      <div className='space-y-6'>
        <div>
          ...
          <button onClick={addEducation}
            ...
            <Plus className='size-4'></Plus>Add Education
          </button>
        </div>
        {data.length === 0 ? (
                ...
                <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                  ...
                  <div className='grid md:grid-cols-2 gap-3'>
                    ...
                    <input value={education.degree || ''} onChange={(e) => updateEducation(index, 'degree', e.target.value)}
                      type="text" placeholder='Degree(eg,Bachelors,Masters)' className='px-3 py-2 text-sm ' />
                    ...
                    <input value={education.graduation_date || ''} disabled={education.is_current} onChange={(e) => updateEducation(index, 'graduation_date', e.target.value)}
                      type="month" className='px-3 py-2 text-sm  ' />
                  </div>

                  <input value={education.gpa || ''} onChange={(e) => updateEducation(index, 'gpa', e.target.value)}
                    type="text" className='px-3 py-2 text-sm ' placeholder='GPA(oprional)' />
